...
# Prior hyper-parameters corresponding to local parameters
lambda.j0 = rep(0.01, J); nu.j0 = p.local + 5; 
m.j0 = list(rep(0,p.local[1]),
            rep(0, p.local[2]),
            rep(0, p.local[3])) 
Psi.j0 = lapply(seq_len(J), function(j){diag(1, p.local[j])})
iPsi.j0 = lapply(seq_len(J), function(j){solve(Psi.j0[[j]])})
...
# List to store local parameter samples (means and variances)
...
for(iter in 2:num_iter){
  ...
  x.j.bar.local = replicate(n = J, list())
  
  for(j in 1:J){
    for(t in 1:L){
      if(n.j.t[[j]][t] == 0){
        x.j.bar.local[[j]][[t]] = rep(0, p.local[j])
      }else{
        x.j.bar.local[[j]][[t]] = rowSums(X.local[[j]][, t.samples[[j]][[iter - 1]] == t, drop = FALSE])/n.j.t[[j]][t]
      }
    }
  }
  
  S.j = replicate(J, list())
  Z.j = replicate(J, list())
  
  lambda.j.hat = replicate(J, list(0)); nu.j.hat = replicate(J, list(0))
  
  for(j in 1:J){
    lambda.j.hat[[j]] = lambda.j0[j] + n.j.t[[j]]
    nu.j.hat[[j]] = nu.j0[j] + n.j.t[[j]]
  }
  
  m.j.hat = replicate(J, list())
  
  for(j in 1:J){
    m.j.hat_temp = matrix(0, nrow = p.local[j], ncol = L)
    S.j_temp_array = array(0, dim = c( p.local[j], p.local[j], L))
    Z.j_temp_array = array(0, dim = c( p.local[j], p.local[j], L))
    for(t in 1:L){
      if(n.j.t[[j]][t] != 0){
        X.local.subset = X.local[[j]][, t.samples[[j]][[iter - 1]] == t, drop = FALSE]
        n.local.subset = ncol(X.local.subset)
        S.j_temp = matrix(0, p.local[j], p.local[j])
        if(n.local.subset > 0){
          for(ii in 1:n.local.subset){
            S.j_temp = S.j_temp + (X.local.subset[, ii] - matrix(x.j.bar.local[[j]][[t]], ncol = 1)) %*% t( (X.local.subset[, ii] - matrix(x.j.bar.local[[j]][[t]], ncol = 1)) )
          }
        }
        
        S.j_temp_array[ , , t] = S.j_temp
        Z.j_temp_array[ , , t] = ( (n.j.t[[j]][t] * lambda.j0[j])/lambda.j.hat[[j]][t] ) * (matrix(x.j.bar.local[[j]][[t]], ncol = 1) - matrix(m.j0[[j]], ncol = 1)) %*% t( (matrix(x.j.bar.local[[j]][[t]], ncol = 1) - matrix(m.j0[[j]], ncol = 1)) )
        
        m.j.hat_temp[, t] = ( (n.j.t[[j]][t] * x.j.bar.local[[j]][[t]]) + (lambda.j0[j] * m.j0[[j]])) / lambda.j.hat[[j]][t]
        
      }
    }
    S.j[[j]] = S.j_temp_array
    Z.j[[j]] = Z.j_temp_array
    m.j.hat[[j]] = m.j.hat_temp
    
  }
  
  
  for(j in 1:J){
    Tau2.j.samples_temp = array(0, dim = c(p.local[j], p.local[j], L))
    M.j.samples_temp = matrix(0, nrow = p.local[j], L)
    for(t in 1:L){
      Tau2.j.samples_temp[,,t] = stats::rWishart(n = 1, df = nu.j.hat[[j]][t], Sigma = solve(iPsi.j0[[j]] + S.j[[j]][, , t] + Z.j[[j]][, , t]))
      M.j.samples_temp[ , t] = MASS::mvrnorm(n = 1, mu = m.j.hat[[j]][ ,t], Sigma = solve(lambda.j.hat[[j]][t] * Tau2.j.samples_temp[,,t] ))
    }
    M.j.samples[[j]][[iter]] = M.j.samples_temp
    Tau2.j.samples[[j]][[iter]] = Tau2.j.samples_temp
  }
  ...
  for(j in 1:J){
    iTau2.j.samples_temp = array(0, dim = dim(Tau2.j.samples[[j]][[iter]]))
    for(l in 1:dim(Tau2.j.samples[[j]][[iter]])[3]){
      iTau2.j.samples_temp[, , l] = solve(Tau2.j.samples[[j]][[iter]][, , l])
    }
    iTau2.j.samples[[j]] <- iTau2.j.samples_temp
  }
  
  for(j in 1:J){
    t.prob[[j]] = prob_exponent_mv_local(pi_samples = pi.samples[[j]][[iter]], 
                                         X_local = t(X.local[[j]]), 
                                         psi_samples = matrix(M.j.samples[[j]][[iter]], nrow = p.local[j], ncol = L),
                                         Sigma =  iTau2.j.samples[[j]],
...
for(iter in 2:num_iter){
  ...
  for(j in 1:J){
    logsum = logsum + logll_mv_local2(pi_samples = pi.samples[[j]][[iter]], X_local = X.local[[j]], 
                                      psi_samples = M.j.samples[[j]][[iter]],  Sigma = iTau2.j.samples[[j]],
...
################################################################################
index <- replicate(J, list())
for(j in 1:J){
  for(iter in 2:num_iter){
    index[[j]][[iter]] = c(t.samples[[j]][[iter]])
  }
}


samples <- samples.thin
posterior_samples <- replicate(J, matrix(0, nrow = length(samples), ncol = length(index[[j]][samples][[1]])))


for(j in 1:J){
  for(i in 1:length(samples)){
    posterior_samples[i, ,j] = index[[j]][samples][[i]]
  } 
}
...
# CLUSTERING BY MINIMIZING VI
################################################################################
z.estimated_mcclust.local = replicate(J, list())
ARI_Local = 0
for(j in 1:J){
  sim_mat = comp.psm(posterior_samples[,,j])
  best_mcclust = minVI(sim_mat, posterior_samples[,,j], method = "all")
  
  singleton_mcclust = as.numeric(names(which(table(best_mcclust$cl[1, ]) <= 0.0 * length(best_mcclust$cl[1, ]))))
  singleton.index_mmclust = which(best_mcclust$cl[1, ] %in% singleton_mcclust)
  z.estimated_mcclust = best_mcclust$cl[1, ]
  z.estimated_mcclust.local[[j]] = z.estimated_mcclust
  ARI_Local[j] = aricode::ARI(z.estimated_mcclust, t.true[[j]])
